...
[TestFixture]
public class ExpenseCategoriesRequestBuilderTests
{
    public class WhenListingExpenseCategories : BaseTestFixture
    {
        [Test]
        public async Task ShouldSetQueryParameters()
        {
            ...
            string expectedRequestUrl =
                $"https://api.harvestapp.com/v2/expense_categories?is_active={isActive}&updated_since={HttpUtility.UrlEncode(updatedSince.ToString("O"))}&page={page}&per_page={perPage}"
                    .ToLowerInvariant();

            // Act
            await harvestServiceClient.ExpenseCategories.GetAsync(c =>
            {
