...
[TestFixture]
public class InvoicePaymentsRequestBuilderTests
{
    public class WhenListingInvoicePayments : BaseTestFixture
    {
        [Test]
        public async Task ShouldSetQueryParameters()
        {
            ...
            string expectedRequestUrl =
                $"https://api.harvestapp.com/v2/invoices/{invoiceId}/payments?updated_since={HttpUtility.UrlEncode(updatedSince.ToString("O"))}&page={page}&per_page={perPage}"
                    .ToLowerInvariant();

            // Act
            await harvestServiceClient.Invoices[invoiceId].Payments.GetAsync(c =>
            {
