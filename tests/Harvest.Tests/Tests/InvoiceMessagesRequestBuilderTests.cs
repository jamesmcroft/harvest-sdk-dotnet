...
[TestFixture]
public class InvoiceMessagesRequestBuilderTests
{
    public class WhenListingInvoiceMessages : BaseTestFixture
    {
        [Test]
        public async Task ShouldSetQueryParameters()
        {
            ...
            string expectedRequestUrl =
                $"https://api.harvestapp.com/v2/invoices/{invoiceId}/messages?updated_since={HttpUtility.UrlEncode(updatedSince.ToString("O"))}&page={page}&per_page={perPage}"
                    .ToLowerInvariant();

            // Act
            await harvestServiceClient.Invoices[invoiceId].Messages.GetAsync(c =>
            {
