...
[TestFixture]
public class EstimateMessagesRequestBuilderTests
{
    public class WhenListingEstimateMessages : BaseTestFixture
    {
        [Test]
        public async Task ShouldSetQueryParameters()
        {
            ...
            string expectedRequestUrl =
                $"https://api.harvestapp.com/v2/estimates/{estimateId}/messages?updated_since={HttpUtility.UrlEncode(updatedSince.ToString("O"))}&page={page}&per_page={perPage}"
                    .ToLowerInvariant();

            // Act
            await harvestServiceClient.Estimates[estimateId].Messages.GetAsync(c =>
            {
