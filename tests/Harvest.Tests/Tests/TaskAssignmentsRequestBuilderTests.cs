...
[TestFixture]
public class TaskAssignmentsRequestBuilderTests
{
    public class WhenListingTaskAssignments : BaseTestFixture
    {
        [Test]
        public async Task ShouldSetQueryParameters()
        {
            ...
            string expectedRequestUrl =
                $"https://api.harvestapp.com/v2/projects/{projectId}/task_assignments?is_active={isActive}&updated_since={HttpUtility.UrlEncode(updatedSince.ToString("O"))}&page={page}&per_page={perPage}"
                    .ToLowerInvariant();

            // Act
            await harvestServiceClient.Projects[projectId].TaskAssignments.GetAsync(c =>
            {
