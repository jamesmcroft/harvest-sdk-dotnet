...
[TestFixture]
public class MyTaskAssignmentsRequestBuilderTests
{
    public class WhenListingTaskAssignments : BaseTestFixture
    {
        ...
        public async Task ShouldSetQueryParameters()
        {
            ...
            string expectedRequestUrl =
                $"https://api.harvestapp.com/v2/task_assignments?is_active={isActive}&updated_since={HttpUtility.UrlEncode(updatedSince.ToString("O"))}&page={page}&per_page={perPage}"
                    .ToLowerInvariant();

            // Act
            await harvestServiceClient.TaskAssignments.GetAsync(c =>
            {
