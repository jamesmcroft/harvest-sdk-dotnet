...
[TestFixture]
public class MyUserAssignmentsRequestBuilderTests
{
    public class WhenListingUserAssignments : BaseTestFixture
    {
        ...
        public async Task ShouldSetQueryParameters()
        {
            ...
            string expectedRequestUrl =
                $"https://api.harvestapp.com/v2/user_assignments?user_id={userId}&is_active={isActive}&updated_since={HttpUtility.UrlEncode(updatedSince.ToString("O"))}&page={page}&per_page={perPage}"
                    .ToLowerInvariant();

            // Act
            await harvestServiceClient.UserAssignments.GetAsync(c =>
            {
