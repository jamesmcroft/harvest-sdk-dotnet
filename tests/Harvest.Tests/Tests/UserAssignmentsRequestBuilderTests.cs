...
[TestFixture]
public class UserAssignmentsRequestBuilderTests
{
    public class WhenListingUserAssignments : BaseTestFixture
    {
        [Test]
        public async Task ShouldSetQueryParameters()
        {
            ...
            string expectedRequestUrl =
                $"https://api.harvestapp.com/v2/projects/{projectId}/user_assignments?user_id={userId}&is_active={isActive}&updated_since={HttpUtility.UrlEncode(updatedSince.ToString("O"))}&page={page}&per_page={perPage}"
                    .ToLowerInvariant();

            // Act
            await harvestServiceClient.Projects[projectId].UserAssignments.GetAsync(c =>
            {
